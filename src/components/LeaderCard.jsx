const LeaderCard = ({ name, title, description, image }) => {
  return (
    <div
      className=" backdrop-blur-sm rounded-2xl p-6 border border-cyan-500/30"
      style={{
    ...
    >
      ...
      <div className="absolute inset-0 rounded-[16px] p-[1px] z-0"
        style={{
      ...
      {/* Name and Title */}
      <div className=" rounded-[16px] p-4 mb-6 relative z-10" style={{
        background: `linear-gradient(0deg, rgba(0, 0, 0, 0.33), rgba(0, 0, 0, 0.33)),
linear-gradient(0deg, rgba(255, 255, 255, 0.2), rgba(255, 255, 255, 0.2))`,
      }}>
        {/* Gradient border using pseudo */}
        ...
        <p className="text-[#00AEEF] text-sm">{title}</p>
      ...
      {/* Description */}
      <p className=" text-[16px] text-[#FFFFFF] leading-relaxed text-center">{description}</p>
    </div>
  );
};
