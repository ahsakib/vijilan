  ...
  return (
    <div className='w-full fixed z-50 top-0'>
      ...
      <header className={`mx-auto h-[96px] flex items-center transition-all duration-300 ease-in-out 
        ...
        px-4 md:px-6 lg:px-0 lg:w-[1160px] rounded-[24px] mt-4 backdrop-blur-sm border-b border-slate-700`}
        style={{
          background: scrolled
      ...
      >
        ...
        {/* Mobile Menu */}
        {menuOpen && (
          <div className={`fixed top-[96px] left-0 w-full rounded-[24px] bg-[#082235] px-6 py-4 z-40 rounded-b-[24px] transition-all duration-300 ease-in-out
...
//       <header
//         className={`mx-auto h-[96px] flex items-center transition-all duration-300 ease-in-out
//         ${scrolled ? 'bg-opacity-50 bg-white' : 'bg-transparent'}
//         px-4 md:px-6 lg:px-0 lg:w-[1160px] rounded-[24px] mt-4 backdrop-blur-sm border-b border-slate-700`}
//         style={{
//           background: scrolled
...
//             <button className="text-white text-[14px] px-[16px] py-[8px] font-semibold rounded-lg" style={{
//               background: "linear-gradient(90deg, #F89B29 0%, #FF0F7B 186.51%)",
//             }}>
//               BOOK A CALL
//             </button>
//             <button onClick={toggleMenu} className="text-white">
//               {menuOpen ? <X className="w-6 h-6" /> : <MoreVertical className="w-6 h-6" />}
//             </button>
//           </div>
//         </div>

//         {/* Mobile Menu Slide Down */}
//         <div className={`lg:hidden absolute top-[96px] rounded-[24px] left-0 w-full bg-[#082235] px-6 py-4 z-40 transition-all duration-300 ease-in-out
//           ${menuOpen ? 'translate-y-0 opacity-100 max-h-[500px]' : '-translate-y-10 opacity-0 max-h-0 overflow-hidden'}`}>
//           <nav className="flex flex-col space-y-4">
//             {['Solutions', 'Platform', 'Customers', 'Insights'].map((item) => (
//               <a key={item} href="#" className="text-white text-[16px] flex items-center justify-between">
//                 {item}
